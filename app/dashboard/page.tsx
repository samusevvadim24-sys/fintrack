...
export default function Dashboard() {
  ...
  const handleOperationClick = () => {
    if (!longPressTriggered.current) setActiveTab("Операции");
    longPressTriggered.current = false;
  };
  ...
  return (
    <main className="min-h-dvh bg-[#f7f9fa] text-slate-950">
      ...
      {menuOpen && (
        <button type="button" aria-label="Закрыть меню" onClick={() => setMenuOpen(false)} className="fixed inset-0 z-30 h-full w-full cursor-default bg-transparent" />
      )}
      ...
      {menuOpen && (
        <div className="fixed bottom-[92px] left-1/2 z-40 w-[190px] -translate-x-1/2 overflow-hidden rounded-[20px] border border-slate-100 bg-white p-1.5 shadow-[0_14px_45px_rgba(15,23,42,0.16)]">
          {operationItems.map((item) => (
            <button key={item.label} type="button" onClick={() => { setActiveTab(item.label); setMenuOpen(false); }} className="flex w-full items-center gap-3 rounded-[14px] px-4 py-3 text-left text-sm font-semibold text-slate-700 transition-colors hover:bg-[#67D1DE]/10 active:scale-[0.98]">
              <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#67D1DE]/12 text-lg text-[#67D1DE]">{item.icon}</span>
              {item.label}
            </button>
          ))}
        </div>
        ...
        <div className="relative flex h-[76px] items-center justify-between rounded-[28px] border border-slate-100 bg-white px-5 shadow-[0_12px_40px_rgba(15,23,42,0.10)] backdrop-blur-xl">
          <div className="flex w-full items-center justify-between pr-10">
            {tabs.slice(0, 2).map((tab) => (
              <button key={tab.label} type="button" onClick={tab.label === "Операции" ? handleOperationClick : () => setActiveTab(tab.label)} onPointerDown={tab.label === "Операции" ? startLongPress : undefined} onPointerUp={tab.label === "Операции" ? endLongPress : undefined} onPointerCancel={tab.label === "Операции" ? endLongPress : undefined} onPointerLeave={tab.label === "Операции" ? endLongPress : undefined} className={`flex min-w-[64px] flex-col items-center gap-1.5 rounded-2xl px-2 py-2 transition-all duration-200 active:scale-90 ${activeTab === tab.label ? "text-[#67D1DE]" : "text-slate-400 hover:text-[#67D1DE]"}`}>
